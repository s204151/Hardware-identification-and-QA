...
def train(opt, show_number = 2, amp=False):
    ...
    if opt.saved_model != '':
        pretrained_dict = torch.load(opt.saved_model,map_location=torch.device('cuda'))
    ...
    #print('CTC in prediction:', 'CTC' in opt.Prediction)
        ...
        if amp:
            with autocast():
                ...
                if 'CTC' in opt.Prediction:
                    ...
                    cost = criterion(preds.view(-1, preds.shape[-1]), target.contiguous().view(-1)) #DEBUG
